...
#if defined(NDEBUG)
# error "Negancoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcd4ad23b8b61f19a3540f4f8f506d9b26063b261499311a9d7c2b55fd5ffcf48");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Negancoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Negancoin Signed Message:\n";
...
{
    // Negancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Negancoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Negancoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Negancoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Negancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Negancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x125b4330d9e448892c8045603a45b5e82dda2d5a8b7aabf5d87becdd5eea086f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "we are all negan";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0417cf8b19727e93ce760515fc794a8c8870cfa78248c7877d9ec629abedf22daf90f1ae0201350dee38613f3f47ba9c8bf13a071086701e4fa2162032d35978ee") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524681383;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084615484;

        if (fTestNet)
        {
            block.nTime    = 1524681367;
            block.nNonce   = 386149423;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5f8c3f67e3385fd02a090176e79b8615774f6c70a6c6bde5cd1cf14b25d4b851"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Negancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NegancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NegancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NegancoinMiner(CWallet *pwallet)
{
    printf("NegancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("negancoin-miner");
    ...
    try { loop {
        ...
        printf("Running NegancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NegancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NegancoinMiner, pwallet));
}
